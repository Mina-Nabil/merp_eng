...
namespace M_ERP
{
    public partial class BuyInvoiceForm : Form
    {
        ...
        private int invoiceFormNo = Varibles.buyInvoiceForm;
        private double supplierBalance = 0;
        ...
        private void BuyInvoiceForm_Load(object sender, EventArgs e)
        {
        }


        public BuyInvoiceForm()
        {
            InitializeComponent();
        ...
        private void fillSupplierName()
        {
            try
            {
                ...
                String sqlCommand = "SELECT SPLR_ID, SPLR_NAME FROM merp.suppliers";
        ...
        {
            supplierNameComboBox.Enabled = tf;
        ...
        private void loadItemBuyPRice()
        {
            try
            {
                ...
                String query = "SELECT PRD_BUYPRICE FROM merp.products WHERE PRD_ID = " + prdNameId + " ;";
                ...
                while (reader.Read())
                {
                    try
                    {
                        price = reader.GetDouble("PRD_BUYPRICE");
                    }
                    catch (Exception ex)
                    {
                        price = 0;
        ...
        private void loadSupplierBalance()
        {
            try
            {
                ...
                supplierBalance = splrBalance;

                supplierBalanceTextBox.Text = supplierBalance.ToString("0.00");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void supplierNameComboBox_TextChanged(object sender, EventArgs e)
        {
            ...
            supplierBalanceTextBox.Text = "" + supplierBalance;
            groupBox2.Enabled = false;
            payNowRadioButton.Checked = true;
        }

        private void DiscoutTextBox_TextChanged(object sender, EventArgs e)
        {
        ...
        private void showFormDetails()
        {
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    supplierNameComboBox.Text = prsn_Name;
                    ...
                    supplierBalanceTextBox.Text = clint_blnc.ToString("0.00");
        ...
        private void saveInvoiceBtn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                selected = Convert.ToInt32(supplierNameComboBox.SelectedValue);
                String q;
                payingWay = 0;
                payingValue = 0;
                ...
                if (selected == 0)
                {
                    q = "INSERT INTO `merp`.`invoices` (`INV_ID`, `INV_DATE`, `INV_TYPE_ID`, `INV_TOTAL`, `INV_PRSN_NAME`, `DISCOUNT`, `INV_PAID`, `INV_FLAG`, `INV_CRNTBLNCE`, `INV_USER_ID`) VALUES ('" + invoiceCodeComboBox.Text + "', '" + dateTimePicker1.Value.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + invoiceFormNo + "', '" + Convert.ToDouble(netTotalInvoicePrice.Text) + "' , '" + supplierNameComboBox.Text + "' , '" + Convert.ToDouble(DiscoutTextBox.Text) + "' , '" + payingValue + "' , " + payingWay + " , '" + Convert.ToDouble(supplierBalanceTextBox.Text) + "' , '" + Varibles.getUserId() + "' );";

                }
                else
                {
                    q = "INSERT INTO `merp`.`invoices` (`INV_ID`, `INV_DATE`, `INV_TYPE_ID`, `INV_TOTAL`, `INV_PRSN_NAME`, `DISCOUNT`, `INV_SPLR` , `INV_PAID`, `INV_FLAG`, `INV_CRNTBLNCE`, `INV_USER_ID`) VALUES ('" + invoiceCodeComboBox.Text + "', '" + dateTimePicker1.Value.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + invoiceFormNo + "', '" + Convert.ToDouble(netTotalInvoicePrice.Text) + "' , '" + supplierNameComboBox.Text + "' , '" + Convert.ToDouble(DiscoutTextBox.Text) + "' , '" + selected + "' , '" + payingValue + "' , " + payingWay + " , '" + Convert.ToDouble(supplierBalanceTextBox.Text) + "' , '" + Varibles.getUserId() + "' );";
            ...
            for (int i = 0; i < dataGridView1.RowCount; i++)
            {
                int ProdCurrentBlnc = getItemBalance2(Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value));
                ...
                try
                {
                    ...
                    inv.addToLog(Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value), Convert.ToInt32(dataGridView1.Rows[i].Cells[3].Value), 0, dateTimePicker1.Value, desc, "فاتورة شراء", Varibles.user_id, Convert.ToInt32(invoiceCodeComboBox.Text), Convert.ToDouble(dataGridView1.Rows[i].Cells[4].Value), 0, invoiceFormNo,ProdCurrentBlnc);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
                ...
                try //update buy price
                {
                    Varibles.setConnection();
                    cmd = Varibles.getCommand();


                    String sqlCommand = "update merp.products set PRD_BUYPRICE = '" + Convert.ToDouble(dataGridView1.Rows[i].Cells[4].Value) + "' WHERE PRD_ID = '" + Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value) + "'" + ";";
            ...
            if (payingWay == 1)
            {
                try
                {
                    ...
                    double paymentOnHim = Convert.ToDouble(supplierBalanceTextBox.Text) + toBePayed;

                    Varibles.setConnection();
                    cmd = Varibles.getCommand();


                    String sqlCommand = "UPDATE `merp`.`suppliers` SET `SPLR_BLNCE`= '" + paymentOnHim + "' WHERE `SPLR_ID`= '" + selected + "' ;";
            ...
            try
            {
                ...
                tr.addToTreasury(0, payingValue, dateTimePicker1.Value, desc, "فاتورة شراء", TreasuryEntry.TE_OUT, selected, 0, Convert.ToDouble(supplierBalanceTextBox.Text));
        ...
        */
        ...
        private void printProfitReport()
        {
            try
            {
                ...
                System.IO.File.WriteAllBytes(sPath, Properties.Resources.buyReciept);
                ...
                xlWorkSheet.Cells[10, 6] = supplierNameComboBox.Text;
                ...
                xlWorkSheet.Range[xlWorkSheet.Cells[i + 7, 5], xlWorkSheet.Cells[i + 7, 5]].Font.Size = 14;
                xlWorkSheet.Range[xlWorkSheet.Cells[i + 7, 5], xlWorkSheet.Cells[i + 7, 5]].WrapText = true;
                xlWorkSheet.Range[xlWorkSheet.Cells[i + 7, 5], xlWorkSheet.Cells[i + 7, 5]].HorizontalAlignment = Excel.XlHAlign.xlHAlignRight;
                xlWorkSheet.Cells[i + 7, 5] = netTotalInvoicePrice.Text;



                xlWorkSheet.Range[xlWorkSheet.Cells[i + 7, 6], xlWorkSheet.Cells[i + 7, 6]].Font.Size = 14;
                xlWorkSheet.Cells[i + 7, 6] = "الصافي :";
                ...
                xlWorkSheet.Range[xlWorkSheet.Cells[i + 9, 2], xlWorkSheet.Cells[i + 9, 4]].Select();
                xlWorkSheet.Range[xlWorkSheet.Cells[i + 9, 2], xlWorkSheet.Cells[i + 9, 4]].Merge(true);
                xlWorkSheet.Range[xlWorkSheet.Cells[i + 9, 2], xlWorkSheet.Cells[i + 9, 4]].Font.Size = 14;
                xlWorkSheet.Range[xlWorkSheet.Cells[i + 9, 2], xlWorkSheet.Cells[i + 9, 4]].WrapText = true;
                xlWorkSheet.Range[xlWorkSheet.Cells[i + 9, 2], xlWorkSheet.Cells[i + 9, 4]].HorizontalAlignment = Excel.XlHAlign.xlHAlignRight;
                xlWorkSheet.Range[xlWorkSheet.Cells[i + 9, 2], xlWorkSheet.Cells[i + 9, 4]].Value = "شكرا لتعاملكم معنا";
