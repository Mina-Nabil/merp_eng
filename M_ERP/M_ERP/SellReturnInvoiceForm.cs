...
namespace M_ERP
{
    public partial class SellReturnInvoiceForm : Form
    {
        ...
        private int invoiceFormNo = Varibles.sellReturnInvoiceForm;
        ...
        private void SellReturnForm_Load(object sender, EventArgs e)
        {
        }
        ...
        private void saveInvoiceBtn_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dataGridView1.RowCount; i++)
            {
                try //check sold
                {
                    ...
                    if (isInSold(pr_id))
                    {
                        sold_balance = balanceInSold(pr_id);

                        if (pr_balance > sold_balance)
                        {
                            MessageBox.Show("خطأ" + " " + "العدد الموجود من هذا الصنف" + "\n" + pr_name + "\n" + "في المبيعات" + " " + sold_balance);
                            return;
                        }
            ...
            for (int i = 0; i < dataGridView1.RowCount; i++)
            {
                ...
                try
                {
                    List<StockItem> soldItemsList = new List<StockItem>();
                    int pr_id = Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value);
                    double pr_price = Convert.ToDouble(dataGridView1.Rows[i].Cells[4].Value);
                    int pr_balance = Convert.ToInt32(dataGridView1.Rows[i].Cells[3].Value);
                    //String pr_name = Convert.ToString(dataGridView1.Rows[i].Cells[2].Value);
                    ...
                    try
                    {
                        String query = "SELECT * FROM sold_items where sold_items.prod_id = '" + pr_id + "' ORDER BY sold_items.buy_price DESC ";
                        ...
                        while (reader.Read())
                        {
                            ...
                            sold_price = reader.GetDouble("buy_price");
                            sold_balance = reader.GetInt32("count");

                            StockItem st = new StockItem(sold_balance, sold_price);
                            soldItemsList.Add(st);
                    ...
                    for (int ii = 0; ii < soldItemsList.Count; ii++)
                    {
                        int ProdCurrentBlnc = getItemBalance2(pr_id);
                        StockItem st = soldItemsList[ii];
                        sold_balance = st.getCount();
                        sold_price = st.getBuyPrice();
                        ...
                        if (pr_balance > sold_balance)
                        {
                            currentBlnc = sold_balance;
                            pr_balance -= sold_balance;
                            deleteFromSold(pr_id, sold_price);

                        }
                        else
                        {
                            currentBlnc = pr_balance;
                            int new_balance = countInSold(pr_id, sold_price) - pr_balance;
                            ...
                            {
                                updateSold(pr_id, new_balance, sold_price);
                            }

                            finishReader = true;
                        }
                        stockCount = currentBlnc;
                        if (isInStock(pr_id, sold_price))
                        {
                            stockCount += countInStock(pr_id, sold_price);

                            updateStock(pr_id, stockCount, sold_price);

                        }
                        else
                        {
                            insertInStock(pr_id, stockCount, sold_price);
                        }


                        String desc = "فاتورة مرتجع بيع رقم - " +
                        ...
                        inv.addToLog(Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value), currentBlnc, 0, dateTimePicker1.Value, desc, "فاتورة مرتجع بيع", Varibles.user_id, Convert.ToInt32(invoiceCodeComboBox.Text), sold_price, pr_price, invoiceFormNo,ProdCurrentBlnc);
                ...
                /*try
                {
                    ...
                    inv.addToLog(Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value), Convert.ToInt32(dataGridView1.Rows[i].Cells[3].Value), 0, dateTimePicker1.Value, desc, "فاتورة مرتجع بيع", Varibles.user_id, Convert.ToInt32(invoiceCodeComboBox.Text));
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
            ...
            try
            {
                ...
                tr.addToTreasury(0, payingValue, dateTimePicker1.Value, desc, "فاتورة مرتجع بيع", TreasuryEntry.TE_OUT, 0, selected, (-1*Convert.ToDouble(clientBalanceTextBox.Text)));
        ...
        private void deleteFromSold(int prdId, double prdBuyPrice)
        {

            String query = "Delete From sold_items where sold_items.prod_id = '" + prdId + "' AND sold_items.buy_price = '" + prdBuyPrice + "'";
        ...
        private bool isInSold(int prdId)
        {
            int count = 0;
            try
            {
                String query = "SELECT COUNT(*) AS count FROM sold_items where sold_items.prod_id = '" + prdId + "'";
        ...
        private int balanceInSold(int prdId)
        {

            int count = 0;
            try
            {
                String query = "SELECT SUM(sold_items.count) AS count FROM sold_items where sold_items.prod_id = '" + prdId + "' ";
        ...
        private void printProfitReport()
        {
            try
            {
                ...
                System.IO.File.WriteAllBytes(sPath, Properties.Resources.SaleRetRec);
                ...
                xlWorkSheet.Range[xlWorkSheet.Cells[i + 9, 8], xlWorkSheet.Cells[i + 7, 6]].Font.Size = 14;
