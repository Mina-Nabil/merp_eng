...
namespace M_ERP
{
    public partial class BuyingReturnInvoiceForm : Form
    {
        ...
        private int invoiceFormNo = Varibles.buyReturnInvoiceForm;
        ...
        private void BuyingReturnInvoiceForm_Load(object sender, EventArgs e)
        {
        }


        public BuyingReturnInvoiceForm()
        {
            InitializeComponent();
        ...
        private void saveInvoiceBtn_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < dataGridView1.RowCount; i++)
            {
                try //check stock
                {
                    ...
                    {
                        stock_balance = countInStock(pr_id, pr_price);
            ...
            for (int i = 0; i < dataGridView1.RowCount; i++)
            {
                ...
                try
                {
                    String desc = "فاتورة مرتجع شراء رقم - " +
                    ...
                    inv.addToLog(Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value), 0, Convert.ToInt32(dataGridView1.Rows[i].Cells[3].Value), dateTimePicker1.Value, desc, "فاتورة مرتجع شراء", Varibles.user_id, Convert.ToInt32(invoiceCodeComboBox.Text), Convert.ToDouble(dataGridView1.Rows[i].Cells[4].Value), 0, invoiceFormNo,ProdCurrentBlnc);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
                ...
                try //update stock
                {
                    ...
                    new_balance = countInStock(pr_id, pr_price) - pr_balance;

                    if (new_balance == 0)
                    {
                        deleteFromStock(pr_id, pr_price);
                    }
                    else
                    {
            ...
            try
            {
                ...
                tr.addToTreasury(payingValue, 0, dateTimePicker1.Value, desc, "فاتورة مرتجع شراء", TreasuryEntry.TE_IN, selected, 0, Convert.ToDouble(supplierBalanceTextBox.Text));
        ...
        private void printProfitReport()
        {
            try
            {
                ...
                System.IO.File.WriteAllBytes(sPath, Properties.Resources.BuyRetRec);
